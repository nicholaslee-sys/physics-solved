...
async function run() {
  // Validate data first — a broken registry/equation/FRQ should fail the build.
  const { errors, warnings } = await validate();
  warnings.forEach((m) => console.warn("  warn: " + m));
  if (errors.length) {
    errors.forEach((m) => console.error("  ERROR: " + m));
    throw new Error(`Data validation failed with ${errors.length} error(s). Fix data/ and rebuild.`);
  }
